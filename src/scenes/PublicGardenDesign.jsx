export default class PublicGardenDesign extends Phaser.Scene {
  constructor() {
    super({ key: 'PublicGardenDesign' });
    this.elements = [];
  ...
  create() {
    ...
    this.add.image(this.cameras.main.centerX, this.cameras.main.centerY, 'publicbackground')
  ...
  createMenuBar() {
    ...
    this.add.text(screenWidth / 2, menuBarHeight / 2, 'Design Your Public Garden!!!', {
      fontSize: '28px',
      fill: '#fff',
      fontFamily: 'Arial',
      fontStyle: 'bold',
    ...
    this.add.image(clockX, menuBarHeight / 2, 'publictimer-clock')
    ...
    const finishButton = this.add.image(screenWidth - 120, menuBarHeight / 2, 'publicfinishButton')
    ...
    const menuButton = this.add.image(50 + 50, menuBarHeight / 2, 'publicbackButton')
      .setInteractive().setDisplaySize(40, 40)
      .on('pointerdown', () => {
        this.buttonTap.play();
        this.scene.start('PublicSelectGardenType');
      });
  ...
  showDragTutorial(overlay) {
    ...
    // Find the example element (publictree5 - Adrak)
    const exampleElement = this.elements.find(el => el.texture.key === 'publictree2');
    if (!exampleElement) return;
  
    // Create animated hand cursor
    const hand = this.add.sprite(exampleElement.x, exampleElement.y, 'publiccursor-hand')
      .setScale(0.5)
      .setDisplaySize(64, 64) // Add this line (adjust values as needed)
      .setDepth(1002);
  
    // Create example element clone for demonstration
    const demoElement = this.add.image(exampleElement.x, exampleElement.y, 'publictree2')
  ...
  showCancelButton(element) {
    if (!element.cancelButton) {
      element.cancelButton = this.add.image(element.x - 60, element.y - 60, 'publiccancelButton')
        .setOrigin(0.5)
        .setInteractive()
        .setDisplaySize(40, 40)
        .setDepth(1000)
...
createActionButtons(element) {
  ...
  const cancelButton = this.add.image(element.x - buttonOffsetX, element.y - buttonOffsetY, 'publiccancelButton')
      .setOrigin(0.5)
      .setInteractive()
      .setDisplaySize(buttonSize, buttonSize)
      .setDepth(1000)
...
cleanupButtons(element) {
  ...
  createElementHolder(x, y, columns, rows, type, startIndex, holderWidth = 300 * 0.7) {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < columns; col++) {
        ...
        const textureKey = `publictree${startIndex + row * columns + col}`;
        const isSpecial = this.specialTrees.has(textureKey);

        this.add.image(px, py, 'publicsquarePlaceholder').setDisplaySize(squareSize, squareSize);
  ...
  createMainImageHolder(x, y, width, height) {
    this.mainImageHolder = this.add.image(x, y, 'publicgardenBg').setDisplaySize(width, height);
